...
app.get("/user/tweets/", authenticationToken, async (request, response) => {
  ...
  const getTweetQuery = `
   SELECT
    ...
    FROM
   user join tweet on user.user_id =tweet.user_id join like on like.tweet_id =tweet.tweet_id join reply on reply.tweet_id =like.tweet_id
   where 
   user.user_id =${getUserId.user_id}`;
  const getTweetIdsArray = await database.all(getTweetQuery);
